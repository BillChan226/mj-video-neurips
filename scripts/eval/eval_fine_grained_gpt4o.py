...
def process_overall_file(json_file_path, result_dir, videos_dir, output_file_name):
    ...
    # Check if output file exists, if so, load existing results
    ...
    if os.path.exists(output_file):
        with open(output_file, 'r') as f:
            ...
            for result in all_results:
                processed_ids.add((result['video_0_uid'], result['video_1_uid']))
    ...
    for item in data:
        ...
        # Unique identifier for this pair of videos
        item_id = (video0_path, video1_path)

        # If this item has already been processed, skip it
        if item_id in processed_ids:
            print(f"Skipping already processed: {video0_path} and {video1_path}")
            continue
        ...
        for category in categories.keys():
            ...
            video_0_rating, video_1_rating, _, response0, response1, prompt = evaluate_videos(video0_path, video1_path, prompt_category)
            ...
            for subcategory in categories[category]["subcategories"].keys():
                ...
                video_0_rating, video_1_rating, _, response0, response1, prompt = evaluate_videos(video0_path, video1_path, prompt_subcategory)
        ...
        # Append the result and update the processed set
        print(result)
        ...
        # Write results back to the output file
...
if __name__ == "__main__": 
    ...
    parser.add_argument('--output_file_name', type=str, default="eval_result_gpt4o.json", help="Name of the output file.")
